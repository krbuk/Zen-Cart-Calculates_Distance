...
/* END: ADD MY CUSTOM DETAILS */
      ...
      $this->products_ordered_attributes . "\n";
	  // nida 	 '<nobr>' .nl2br($this->products_ordered_attributes) .'</nobr>'
      $this->products_ordered_html .=
      '<tr>' . "\n" .
      ...
      '<nobr>' .nl2br($this->products_ordered_attributes) .'</nobr>' .
      '</td>' . "\n" .
  ...
  function send_order_email($zf_insert_id = null, $zf_mode = FALSE) {
    ...
    // nida EMAIL_TEXT_DATE_ORDERED . ' ' . strftime(DATE_FORMAT_LONG) . "\n" .
    EMAIL_TEXT_DATE_ORDERED . ' ' . date("d.m.Y - G:i:s") . "\n" .			
    ...
    // nida $html_msg['INTRO_DATE_ORDERED']    = strftime(DATE_FORMAT_LONG);
    $html_msg['INTRO_DATE_ORDERED']    =  date("d.m.Y - G:i:s");	  
...
//    }
    if ($this->content_type != 'virtual' && !$storepickup) {
       $html_msg['ADDRESS_DELIVERY_DETAIL']    = zen_address_label($_SESSION['customer_id'], $_SESSION['sendto'], true, '', "<br />");
	   $html_msg['ADDRESS_DELIVERY_DETAIL']   .= '<br><a href="https://maps.google.com/maps/search/?api=1&amp;query=' . urlencode($this->delivery['street_address'] . ',' . $this->delivery['city'] .',' . $this->delivery['state'] . ',' . $this->delivery['postcode']) .'" target="map">' .TEXT_MAP_CUSTOMER_ADDRESS .'</a>';		
    } else {
       $html_msg['ADDRESS_DELIVERY_DETAIL']    = $this->customer['firstname'] . ' ' . $this->customer['lastname']; 
    }
	// end distance nida	  
	...
	// nida  
	if (defined('EMAIL_FOOTER_COMPANY_INFO')) $email_order .= "\n-----\n" . EMAIL_FOOTER_COMPANY_INFO . "\n\n";
	$html_msg['EMAIL_FOOTER_COMPANY_INFO']  = EMAIL_FOOTER_COMPANY_INFO; 

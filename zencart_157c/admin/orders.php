...
$optURL = FILENAME_ORDERS . '.php?' . zen_get_all_get_params(['t', 'na', 'ns']);
$listingURL = FILENAME_ORDERS . '.php?' . zen_get_all_get_params(['q', 't', 'na', 'ns']);
//end session nida 
...
<?php } ?>
  ...
  <body>
    ...
    <!-- header_eof //-->
    <!-- nida order melodi -->
      <h1 class="pageHeading"><?php echo TEXT_HEADING_TITLE; ?></h1>
	<?php
	global $db;
	$sql = "SELECT orders_id FROM ". TABLE_ORDERS . " ORDER BY orders_id DESC LIMIT 1;";
	$queryResult = $db->Execute($sql);

	if( !isset($_SESSION['last_order']) )
		$_SESSION['last_order'] = $queryResult->fields['orders_id'];
	//echo 'db last order id: '.$queryResult->fields['orders_id'];
	//echo '<br/> sSession last order id: '.$_SESSION['last_order'];	  
	if( $_SESSION['last_order'] != $queryResult->fields['orders_id'])
	{
		//play cha ching
	//$mediaurl = HTTPS_CATALOG_SERVER.DIR_WS_HTTPS_CATALOG.'media/flute.mp3';
    ?>
		<audio controls autoplay>
		  <source src="<?php echo DIR_WS_CATALOG ?>media/flute.mp3" type="audio/mpeg">
...
//        echo $module->admin_notification($oID);
		...
		// start distance  nida   
		// Google calculater distance table
		global $db, $order;			  
		$customers_check_status = $db->Execute("SELECT customers_id
											  FROM " . TABLE_ORDERS . "
											  WHERE orders_id = " . (int)$_GET['oID']);	
		$customerID = $customers_check_status->fields['customers_id'];

		$distance_query = "SELECT * from " .TABLE_ADDRESS_BOOK . " 
												  WHERE customers_id =  '". $customerID ."' ";
		$distance_result = $db->Execute($distance_query);		  
		foreach ($distance_result as $distance_count) {
		  $addressresut = $distance_count['entry_street_address'];
			if ($order->customer['street_address'] == $addressresut) { $distance_customer = $distance_count['distance']; }	
			if ($order->delivery['street_address'] == $addressresut) { $distance_delivery = $distance_count['distance']; }
			if ($order->billing['street_address']  == $addressresut) { $distance_billing  = $distance_count['distance']; }
		}
		// end distance  nida 		  
        ?>
        <div class="row">
        ...
        <div class="row">
          <div class="col-sm-4">
            <table class="table">
              ...
              <tr>
                <td>&nbsp;</td>
                <td class="noprint"><a href="https://maps.google.com/maps/search/?api=1&amp;query=<?php echo urlencode($order->customer['street_address'] . ',' . $order->customer['city'] . ',' .  $order->customer['state'] . ',' . $order->customer['postcode']); ?>" rel="noreferrer" target="map"><i class="fa fa-map">&nbsp;</i> <u><?php echo TEXT_MAP_CUSTOMER_ADDRESS.'&nbsp; &#40;' .$distance_customer .'km &#41;'; ?></u></a></td>
              </tr>
<?php
...
<?php } ?>
          ...
          <div class="col-sm-4">
            <table class="table">
              ...
              <tr>
                <td>&nbsp;</td>
                <td class="noprint"><a href="https://maps.google.com/maps/search/?api=1&amp;query=<?php echo urlencode($order->delivery['street_address'] . ',' . $order->delivery['city'] . ',' . $order->delivery['state'] . ',' . $order->delivery['postcode']); ?>" rel="noreferrer" target="map"><i class="fa fa-map">&nbsp;</i> <u><?php echo TEXT_MAP_SHIPPING_ADDRESS.'&nbsp; &#40;' .$distance_delivery .'km &#41;'; ?></u></a></td>
              </tr>
<?php
  $address_footer_suffix = '';
...
<?php } ?>
          ...
          <div class="col-sm-4">
            <table class="table">
              ...
              <tr>
                <td>&nbsp;</td>
                <td class="noprint"><a href="https://maps.google.com/maps/search/?api=1&amp;query=<?php echo urlencode($order->billing['street_address'] . ',' . $order->billing['city'] . ',' . $order->billing['state'] . ',' . $order->billing['postcode']); ?>" rel="noreferrer" target="map"><i class="fa fa-map">&nbsp;</i> <u><?php echo TEXT_MAP_BILLING_ADDRESS .'&nbsp; &#40;' .$distance_billing .'km &#41;'; ?></u></a></td>
              </tr>
<?php
  $address_footer_suffix = '';
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br><br><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
                ...
                default:
                  if (isset($oInfo) && is_object($oInfo)) {
					...
					// nida  
                    $contents[] = array('align' => 'text-center', 'text' => '<a href="' . zen_href_link(FILENAME_PRINT_PER_ORDER, 'oID=' . $oInfo->orders_id) . '" target="_blank" class="btn btn-info" role="button">' . IMAGE_ORDERS_PRINT . '</a>');						  
